...
func main() {
	...
	if err != nil {
		fmt.Printf("Failed to get TUN device name: %v", err)
		os.Exit(ExitSetupFailed)
	}
	...
	err = device.Up()
	if err != nil {
		logger.Errorf("Failed to listen on uapi socker: %v", err)
		os.Exit(ExitSetupFailed)
	}
	...
	if err != nil {
		logger.Errorf("Failed to set interface address: %v. %v", err, stderr)
		os.Exit(ExitSetupFailed)
	}
